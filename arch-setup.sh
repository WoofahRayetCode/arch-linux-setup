...
sudo pacman -Sy
sudo pacman -S nvidia lib32-nvidia-utils nvidia-utils nvidia-settings usbmuxd openh264 movit kdenlive partitionmanager mame-tools lutris qbittorrent lib32-mangohud mangohud goverlay wine-staging vulkan-icd-loader lib32-vulkan-icd-loader lib32-vulkan-mesa-layers vulkan-mesa-layers winetricks bleachbit gamemode steam protontricks discord telegram-desktop krita libreoffice-fresh obs-studio android-tools
...
sudo pacman -Sy
yay -S envycontrol noisetorch betterdiscord-installer mediawriter chatall-bin mullvad-vpn standardnotes-bin stream-overlay obs-backgroundremoval gcdemu xpadneo-dkms minecraft-launcher ventoy-bin protonup-qt youtube-music-bin visual-studio-code-bin timeshift noisetorch-git
