...
sudo pacman -Sy
sudo pacman -S usbmuxd openh264 movit kdenlive kalarm partitionmanager mame-tools lutris qbittorrent corectrl lib32-mangohud mangohud goverlay wine-staging vulkan-icd-loader lib32-vulkan-icd-loader amdvlk lib32-amdvlk lib32-vulkan-radeon vulkan-radeon lib32-vulkan-mesa-layers vulkan-mesa-layers winetricks bleachbit gamemode steam discord telegram-desktop krita libreoffice-fresh obs-studio android-tools
...
sudo pacman -Sy
yay -S linux-xanmod-edge-linux-bin-x64v3 mediawriter chatall-bin mullvad-vpn standardnotes-bin openrgb-bin liquidctl-git coolercontrol stream-overlay obs-backgroundremoval caprine google-chrome gcdemu xpadneo-dkms minecraft-launcher ventoy-bin protonup-qt youtube-music-bin visual-studio-code-bin timeshift noisetorch-git

#So sensor stuff can work
sudo sensors-detect

#Start service for CoolerControl
