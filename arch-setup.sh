...
#Lenovo 9i
sudo pacman -S base-devel flatpak nvidia-dkms lib32-nvidia-utils nvidia-utils nvidia-settings ladspa noise-suppression-for-voice alacritty freedownloadmanager neofetch bash-language-server usbmuxd openh264 movit kdenlive partitionmanager mame-tools lutris qbittorrent lib32-mangohud mangohud goverlay wine-staging vulkan-icd-loader lib32-vulkan-icd-loader lib32-vulkan-mesa-layers vulkan-mesa-layers winetricks bleachbit gamemode steam-native-runtime protontricks discord telegram-desktop krita libreoffice-fresh android-tools
...
sudo pacman -Sy
yay -S envycontrol webapp-manager tenacity-git baca-ereader-git coolercontrol betterdiscord-installer-bin chatall-bin mullvad-vpn standardnotes-bin stream-overlay obs-backgroundremoval-git gcdemu xpadneo-dkms minecraft-launcher ventoy-bin protonup-qt youtube-music-bin visual-studio-code-bin timeshift
